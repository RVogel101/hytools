...
def _detect_eastern_armenian_markers(text: str) -> tuple[int, list[str]]:
    """Count Eastern Armenian reform markers and return example issues.
    ...
    for marker, weight in _EASTERN_ARMENIAN_REFORM_MARKERS:
        ...
        if count > 0 and weight > 0:  # Only count positive-weight EA markers
            ea_count += count
            if len(examples) < 3:  # Keep first 3 examples
                examples.append(f"Found '{marker}' ({count}x)")
...
def validate_nayiri_dictionary(text: str) -> tuple[bool, list[str]]:
    ...
    # 1. from armenian_corpus_core.cleaning.armenian_tokenizer import extract_words
