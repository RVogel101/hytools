...
class MorphologicalMetrics:
    ...
    suffix_em_count: int  # Eastern: -եմ (1st singular)
    suffix_em_frequency: float
    suffix_im_count: int  # Western: -իմ (1st singular)
    suffix_im_frequency: float
    suffix_um_count: int  # -ում (various)
    suffix_um_frequency: float
    suffix_an_count: int  # -ան (various)
    suffix_an_frequency: float
    suffix_el_count: int  # -ել (infinitive)
...
class QuantitativeLinguisticsAnalyzer:
    ...
    def _compute_morphological_metrics(
        ...
        Tracks key suffixes that differ between Western and Eastern Armenian.
        """
        total_words = len(words)
        ...
        return MorphologicalMetrics(
            suffix_em_count=em_count,  # Eastern 1st singular
            suffix_em_frequency=round(em_count / total_words, 6) if total_words > 0 else 0,
            suffix_im_count=im_count,  # Western 1st singular
            suffix_im_frequency=round(im_count / total_words, 6) if total_words > 0 else 0,
            suffix_um_count=um_count,
            suffix_um_frequency=round(um_count / total_words, 6) if total_words > 0 else 0,
            suffix_an_count=an_count,
            suffix_an_frequency=round(an_count / total_words, 6) if total_words > 0 else 0,
            suffix_el_count=el_count,  # Infinitive
            suffix_el_frequency=round(el_count / total_words, 6) if total_words > 0 else 0,
        )
    ...
    def _compute_semantic_metrics(self, words: list[str]) -> SemanticMetrics:
        ...
        # Pronoun frequency
        pronouns = ['ես', 'դու', 'նա', 'ինք', 'մենք', 'դուք', 'նրանք', 'հն', 'այն', 'սա']
    ...
    def _compute_contamination_metrics(
        ...
        # Code-switching index: ratio of ambiguous forms
        # (forms that could be -եմ OR -իմ with spelling variance)
        em_forms = sum(1 for w in words if w.endswith("եմ"))
        im_forms = sum(1 for w in words if w.endswith("իմ"))

        total_1sg_forms = em_forms + im_forms
        code_switching = em_forms / max(1, total_1sg_forms)

        # Eastern form ratio: estimated presence of Eastern forms
        eastern_ratio = code_switching

        # Variant ratio average
        variant_ratio = 0.0
        if total_1sg_forms > 0:
            variant_ratio = em_forms / total_1sg_forms

        return ContaminationMetrics(
            code_switching_index=round(code_switching, 6),
            eastern_form_ratio=round(eastern_ratio, 6),
            variant_ratio_avg=round(variant_ratio, 6),
        )
    ...
    def _compute_quality_flags(
        ...
        # Check for Eastern contamination
        if morphological.suffix_em_count > 0:
            issues.append(f"Found {morphological.suffix_em_count} Eastern -եմ forms")
            dialect_purity -= 0.1 * min(1.0, morphological.suffix_em_count / 10)
...
if __name__ == "__main__":
    ...
    sample_text = "Սա մի լավ տուն է։ Մենք հաճախ այստեղ գալիս ենք։"
