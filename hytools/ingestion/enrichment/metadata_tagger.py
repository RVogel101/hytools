...
def _enrich_document(doc: dict, source: str, text: str = "", analyzer: object | None = None) -> dict | None:
    ...
    """
    from hytools.ingestion._shared.helpers import classify_language, compute_wa_score_detailed, compute_script_purity_score, _any_armenian_script
    ...
    if text.strip() and existing.get("text_metrics_date") is None and _any_armenian_script(text):
        try:
            ...
            from hytools.linguistics.lexicon.loanword_tracker import analyze_loanwords
...
def run(config: dict) -> None:
    ...
    with open_mongodb_client(config) as client:
        ...
        try:
            from hytools.linguistics.metrics.text_metrics import QuantitativeLinguisticsAnalyzer as _QLA
            _analyzer = _QLA()
