...
**Primary (MongoDB)**: Use ``cleaning.run_mongodb`` — normalizes and filters
corpus documents in MongoDB. No file I/O. Invoked by scraping runner.

**Legacy (file-based)**: This module's ``run_pipeline`` reads from
``data/raw/**``, writes to ``data/cleaned``, ``data/deduped``, ``data/filtered``.
Deprecated: corpus is stored in MongoDB; use ``run_mongodb`` instead.

Examples
--------

Run MongoDB cleaning (via pipeline):
    python -m scraping.runner run --only cleaning

Run legacy file-based cleaning:
    python -m cleaning.runner run

Status:
    python -m cleaning.runner status
"""
...
def _extract_culturax_jsonl(raw_dir: Path, stage_dir: Path) -> int:
    ...
    Note: CulturaX now writes only to MongoDB. Use mongodb_export stage to
    export MongoDB documents to data/raw; those will be picked up by _copy_tree.
    This function remains for legacy raw_dir/culturax/*.jsonl if present.
    """
    import hashlib

    culturax_dir = raw_dir / "culturax"
    if not culturax_dir.exists():
        return 0

    count = 0
    for jsonl in culturax_dir.rglob("*.jsonl"):
...
def run(config: dict | None = None) -> dict:
    """Entry-point for pipeline runner: run the full cleaning pipeline."""
    return run_pipeline(promote_filtered=True)
...
def _launch_background() -> None:
    cmd = [sys.executable, "-m", "cleaning.runner", "run", "--force-legacy"]
    ...
    print("Status: python -m cleaning.runner status")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(prog="python -m cleaning.runner")
    sub = parser.add_subparsers(dest="command", required=True)

    run_p = sub.add_parser("run", help="Run cleaning pipeline (DEPRECATED: use MongoDB cleaning)")
    run_p.add_argument("--background", action="store_true", help="Launch detached background process")
    run_p.add_argument(
        "--force-legacy",
        action="store_true",
        help="Allow legacy file-based cleaning (data/cleaned, data/deduped, data/filtered). "
             "Default: exit with message to use MongoDB cleaning instead.",
    )
    ...
    if args.command == "run" and not getattr(args, "force_legacy", False):
        print()
        print("Legacy file-based cleaning is deprecated.")
        print("The corpus lives in MongoDB. Use: python -m scraping.runner run --only cleaning")
        print("That runs cleaning.run_mongodb (no data/cleaned, data/deduped, data/filtered).")
        print()
        print("To force legacy file-based cleaning: python -m cleaning.runner run --force-legacy")
        print()
        return
